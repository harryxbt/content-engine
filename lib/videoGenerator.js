...
// Constants - 1080p output (9:16 TikTok format)
...
function escapeText(text) {
  return text
    .replace(/\\/g, '\\\\')
    .replace(/'/g, "\u2019")  // Replace ' with curly apostrophe (safe for ffmpeg)
    .replace(/:/g, '\\:')
    .replace(/\[/g, '\\[')
    .replace(/\]/g, '\\]');
}
...
/**
 * Escape path for ffmpeg drawtext fontfile
 */
 ...
 * Returns { outputPath, metrics: { ffprobeDurationMs, ffmpegDurationMs, totalDurationMs, videoDurationSec } }
 */
async function generateVideo({ inputPath, outputPath, caption, fontPath }) {
  const metrics = { steps: [] };
  const totalStart = Date.now();

  // Get video info with timing
  const ffprobeStart = Date.now();
  const { duration } = await getVideoInfo(inputPath);
  metrics.ffprobeDurationMs = Date.now() - ffprobeStart;
  metrics.steps.push({ step: 'ffprobe', durationMs: metrics.ffprobeDurationMs });

  const trimmedDuration = Math.max(1, duration - TRIM_START - TRIM_END);
  metrics.videoDurationSec = trimmedDuration;
  ...
  const escapedFontFile = escapePath(fontFile);
  ...
  lines.forEach((line, i) => {
    ...
    filterParts.push(
      `[${lastBannerLabel}]drawtext=fontfile=${escapedFontFile}:text='${escapedLine}':fontsize=48:fontcolor=black:x=(w-text_w)/2:y=${y}-th/2[${newLabel}]`
    );
  ...
  const filterFile = path.join(os.tmpdir(), `filter_${Date.now()}.txt`);
  ...
  return new Promise((resolve, reject) => {
    ...
    ffmpeg.on('close', (code) => {
      ...
      if (code === 0) {
        metrics.ffmpegDurationMs = Date.now() - ffmpegStart;
        metrics.totalDurationMs = Date.now() - totalStart;
        metrics.steps.push({ step: 'ffmpeg', durationMs: metrics.ffmpegDurationMs });
